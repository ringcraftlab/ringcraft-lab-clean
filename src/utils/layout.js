...
export function isFoldLayoutMode(layoutMode) {
  return layoutMode === 'fold3' || layoutMode === 'fold4';
}

export function foldCountFromMode(layoutMode) {
...
export function paperOrientationForLayout(layoutMode) {
  return layoutMode === 'portrait' ? 'portrait' : 'landscape';
}

/**
 * A4横に載せる折り帯レイアウト（面ごと画像・左穴1列）
 * @param {number} refillW
 * @param {number} refillH
 * @param {3 | 4} foldCount
 */
export function calcFoldLayout(refillW, refillH, foldCount) {
