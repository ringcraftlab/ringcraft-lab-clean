export function extractBackgroundImageUrl(bgImage: string): string | null {
 ...
 */
export function promoteBackgroundImagesForCapture(root: ParentNode): void {
  root.querySelectorAll('[role="img"]').forEach((node) => {
...
export function prefersMobilePrintFlow(): boolean {
...
export interface BuildPrintPageHtmlParams {
  dataUrl: string
  pxW: number
  pxH: number
  pageWmm: number
...
export function buildPrintPageHtml({
  ...
  pageHmm,
  autoPrint = false,
}: BuildPrintPageHtmlParams): string {
...
export type MobilePdfPrintPlatform = 'ios' | 'android'

/** スマホ PDF 表示時の印刷手順（iOS / Android） */
export function detectMobilePdfPrintPlatform(): MobilePdfPrintPlatform | null {
...
const MOBILE_PDF_PRINT_HINTS: Record<MobilePdfPrintPlatform, string> = {
  ios: '画面下の共有ボタン→「プリント」を選んでください',
  android: '右上のメニュー→「印刷」を選んでください',
}

export interface BuildMobilePdfViewerHtmlParams {
  pdfUrl: string
  platform: MobilePdfPrintPlatform
}
 ...
 */
export function buildMobilePdfViewerHtml({ pdfUrl, platform }: BuildMobilePdfViewerHtmlParams): string {
...
export interface OpenPrintDocumentResult {
  ok: boolean
  win: Window | null
...
export interface OpenSheetPdfBlobOptions {
 ...
 */
export async function openSheetPdfBlob(
  pdfBlob: Blob,
  { revokeAfterMs = 120000 }: OpenSheetPdfBlobOptions = {},
): Promise<OpenPrintDocumentResult> {
  ...
  if (useViewer) {
    ...
    return { ok: false, win: null, printUrl: null }
  }
...
export async function openPrintDocument(
  html: string,
  { autoPrint = false }: OpenPrintDocumentOptions = {},
): Promise<OpenPrintDocumentResult> {
 ...
 */
export function printViaHiddenIframe(
  html: string,
  { autoPrint = false }: OpenPrintDocumentOptions = {},
): Promise<OpenPrintDocumentResult> {
...
export function nextPaintFrames(count = 2): Promise<void> {
  return new Promise((resolve) => {
    let left = count
    const tick = () => {
      left -= 1
...
export function waitForImagesLoaded(root: ParentNode | null, timeoutMs = 5000): Promise<void> {
